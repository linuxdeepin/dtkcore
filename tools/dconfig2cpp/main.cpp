...
int main(int argc, char *argv[]) {
    ...
    QCommandLineOption cppPropertyOption(QStringList() << QStringLiteral("use-qproperty"),
                                         QStringLiteral("Generate Qt/C++ properties use QProperty"));
    ...
    if (className.isEmpty()) {
        ...
        className = QStringLiteral("dconfig_") + jsonFileName.replace('.', '_');
    }
    ...
    for (auto it = contents.begin(); it != contents.end(); ++it) {
        ...
        if (parser.isSet(cppPropertyOption)) {
            headerStream << " BINDABLE bindable" << capitalizedPropertyName;
        }
        headerStream << ")\n";
    }
    headerStream << "public:\n";
    ...
    for (int i = 0; i < properties.size(); ++i) {
        ...
        if (parser.isSet(cppPropertyOption)) {
            headerStream << "        p_" << property.propertyName << ".setValue(value);\n";
        } else {
            headerStream << "        auto oldValue = p_" << property.propertyName << ";\n"
    ...
    headerStream << "    void initializeInMainThread() {\n";
    for (int i = 0; i < properties.size(); ++i) {
        const Property &property = properties[i];
        headerStream << "        connect(this, &" << className << "::" << property.propertyName << "Changed" << ", this, [this] {\n"
                     << "            Q_ASSERT(QThread::currentThread() == this->thread());\n"
                     << "            if (m_propertyChangedIsFromConfig) return;\n"
                     << "            markPropertySet(" << i << ");\n"
                     << "            if (auto config = m_config.loadRelaxed()) {\n"
                     << "                QMetaObject::invokeMethod(config, [this]() {\n"
                     << "                    Q_ASSERT(QThread::currentThread() == m_config.loadRelaxed()->thread());\n"
                     << "                    m_config.loadRelaxed()->setValue(QStringLiteral(\"" << property.propertyName << "\"), " + property.propertyName + "());\n"
                     << "                });\n"
                     << "            }\n"
                     << "        }, Qt::DirectConnection);\n";
    }
    ...
    for (const Property &property : std::as_const(properties)) {
        ...
        headerStream << "            QMetaObject::invokeMethod(this, [this, newValue]() {\n"
                     << "                Q_ASSERT(QThread::currentThread() == this->thread());\n"
                     << "                m_propertyChangedIsFromConfig = true;\n";
        if (parser.isSet(cppPropertyOption)) {
        ...
        headerStream << "                m_propertyChangedIsFromConfig = false;\n";
        headerStream << "            });\n";
        headerStream << "            return;\n";
        headerStream << "        }\n";
    }
    ...
    headerStream << "    QAtomicPointer<DTK_CORE_NAMESPACE::DConfig> m_config = nullptr;\n"
                 << "    bool m_propertyChangedIsFromConfig = false;\n";
    ...
    for (const Property &property : std::as_const(properties)) {
        ...
        if (parser.isSet(cppPropertyOption)) {
            headerStream << "    Q_OBJECT_BINDABLE_PROPERTY_WITH_ARGS(" << className << ", "
                         << property.typeName << ", p_" << property.propertyName << ", { "
                         << jsonValueToCppCode(property.defaultValue) << " }, "
                         << "&" << className << "::" << property.propertyName << "Changed)\n";
        } else {
