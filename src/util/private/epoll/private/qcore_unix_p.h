...
Q_DECLARE_TYPEINFO(epoll_data, Q_PRIMITIVE_TYPE);
...
inline struct timespec qt_gettime() Q_DECL_NOTHROW
{
    qint64 sec, frac;
    do_gettime(&sec, &frac);
...
Q_CORE_EXPORT int qt_safe_epoll(int epoll_fd, struct epoll_event *evs, nfds_t nfds, const struct timespec *timeout_ts);

static inline int qt_epoll_msecs(int epoll_fd, struct epoll_event *evs, nfds_t nfds, int timeout)
{
    ...
    return qt_safe_epoll(epoll_fd, evs, nfds, pts);
}

static inline struct epoll_event qt_make_epollfd(int fd, int events)
{
...
static inline void qt_epollfd_ctl(int type, int epfd, int fd, int op)
{
    uint32_t events = [](int type) -> uint32_t {
        if (type == 0) // QSocketNotifier::Type::Read
            return EPOLLIN;
        if (type == 1) // QSocketNotifier::Type::Write
            return EPOLLOUT;
        // QSocketNotifier::Type::Exception
        return EPOLLPRI | EPOLLHUP | EPOLLERR;
    }(type);
    epoll_event ev = qt_make_epollfd(fd, events);
    int err = epoll_ctl(epfd, op, fd, &ev);
    Q_ASSERT(err == 0);
    if (err == -1) {
        qWarning("Error, EPOLL_CTL_ADD: %s", strerror(errno));
    }
}
